...
4. **确定批次目录**：
   ...
   **检测逻辑：**
   ...
   1. **从 PRD 路径提取**（最常用）：
      - 如果 PRD 路径中包含 `<person>_YYYYMMDD` 或 `<person>_YYYYMMDD_N` 格式的目录名（`<person>` 为第 3 小节解析出的个人标识），直接用它作为批次目录名
      - 例：`Prd/亲亲创客合伙人二期/ben_20260630_2/` → 批次目录名 = `ben_20260630_2`（示例，实际取配置的 person 值）
      - Claude 输出镜像创建同名批次目录
...
2. **项目分组**：取 `Prd/` 下第一层目录名作为项目名（如 `Prd/亲亲创客合伙人二期/ben_20260630/` → `亲亲创客合伙人二期`）。在「## ✅ 已完成」下找同名 `### <项目名>` 小节；没有则新建该小节和表头。
3. **写行**：
