...
1. **检查恢复状态**（最先执行）：
   - 如果传入了 `--resume` 参数，先按已有配置（项目优先，其次全局）读取 `person`，用批次目录（`YYYYMMDD` 或旧格式 `<person>_YYYYMMDD`）定位最近的 Claude 输出会话目录；读不到 `person` 时按子步骤 3 的规则解析后再定位
...
5. **确定输出目录**：
   ...
   **情况 A：有项目结构**（路径形如 `root/Prd/项目名/<状态>/YYYYMMDD/`）
   - 输出到 `root/Claude/项目名/YYYYMMDD/`
   - 例：`ben-workspace/Prd/sui-yin-ji/已交付/20260708/` → `ben-workspace/Claude/sui-yin-ji/20260708/`

   **情况 B：没有项目结构**（普通目录或文件）
   - 输出到 `<PRD目录>/../Claude/YYYYMMDD/`
   - 例：`D:/work/prd/` → `D:/work/Claude/20260708/`

   **情况 C：对话输入**（无文件）
   - 输出到 `./output/Claude/YYYYMMDD/`（当前工作目录）
...
2. **项目分组**：取 `Prd/` 下第一层目录名作为项目名（如 `Prd/sui-yin-ji/已交付/20260708/` → `sui-yin-ji`；`Prd/ben-plugins/01dev-flow/已交付/20260714/` → `ben-plugins`，子项目归入父项目分组）。在「## ✅ 已完成」下找同名 `### <项目名>` 小节；没有则新建该小节和表头。
3. **写行**：
