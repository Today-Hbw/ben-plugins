...
**项目分组**：取 `Prd/` 下第一层目录名作为项目名（如 `Prd/ben-plugins/01dev-flow/已交付/20260714/需求.md` → `ben-plugins`，子项目归入父项目分组；`Prd/sui-yin-ji/已交付/20260708/需求.md` → `sui-yin-ji`）。在「## ✅ 已完成」下找同名 `### <项目名>` 小节；没有则新建该小节和表头。
